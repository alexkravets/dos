...
class Operation {
  ...
  static createMutationSchema(source) {
    const { id } = this
    return new Schema(`${id}InputMutation`, source)
  }

  static cloneMutationSchema(Component, options) {
    const { id } = this
    const schema = Component.schema.clone(`${id}InputMutation`, options)
    return schema
  }
