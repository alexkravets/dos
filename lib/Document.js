...
class Document extends Component {
  ...
  static get documentSchema() {
    ...
    if (!schema) {
      throw new Error(`${this.name} schema is not found`)
    }
  ...
  static async create(context, attributes) {
    ...
    // TODO: Move this to _create (document is not created without this field)
    //       or add an exception.
  ...
  static async read(context, query, options = {}) {
    ...
    if (id) {
      ...
      const { objects } = await this.index(context, query, { ...options, limit: 1 })
      object = objects[0]
  ...
  static async update(context, query, attributes) {
    ...
    if (!id) {
      const object = await this.read(context, query)
      id = object.id
    }

    // TODO: Remove _isDeleted
  ...
  static async delete(context, query) {
    ...
    // TODO: Consider adding support of deletedAt and deletedBy fields.
    // attributes.deletedAt = new Date().toJSON()
    // if (context.userId) {
    //   attributes.deletedBy = context.userId
    // }
  ...
  async save(parameters = {}) {
    ...
    if (isCreated) {
      ...
      object = await this.constructor.create(this.context, attributes)
