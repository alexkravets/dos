...
class Http {
  ...
  _logRequest(req) {
    ...
    if (isQuery) {
      ...
      signale.info(chalk`{cyan ${req.method}} {yellow ${req.originalUrl}} {dim ${parameters}}`)

    } else {
      const parameters = JSON.stringify({ query: req.query, mutation: req.mutation }, null, 2)
      signale.info(chalk`{magenta ${req.method}} {yellow ${req.originalUrl}} {dim ${parameters}}`)
  ...
  _logResponse(req, statusCode, result) {
    ...
    if (statusCode >= 500) {
      signale.error(chalk`{red ${status}} {dim ${json}}`)

    } else if (statusCode >= 400) {
      signale.debug(chalk`{yellow ${status}} {dim ${json}}`)

    } else {
      if (isQuery) {
        signale.debug(chalk`{cyan ${status}} {dim ${json}}`)

      } else {
        signale.debug(chalk`{magenta ${status}} {dim ${json}}`)
  ...
  listen() {
    ...
    return new Promise(resolve => this.http.listen(this.port, () => {
      signale.info(chalk`Server started at: {red http://${this.host}}`)
      resolve(this)
    }))
  }
}
