...
class Http {
  ...
  _parseBody(req, callback) {
    ...
    const onAborted = () => {
      ...
      const error = this._buildError(400, 'RequestAborted', new Error('Request aborted'))
      done(error)
    }
    ...
    const onError = err => {
      if (isComplete) { return }
      const error = this._buildError(400, 'StreamError', err)
      done(error)
    }
  ...
  _handler(req, res) {
    this._parseBody(req, async(error) => {
      ...
      try {
        ...
        result = this._buildError(500, 'InternalServerError', error)
