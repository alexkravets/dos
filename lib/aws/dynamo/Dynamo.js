'use strict'

const uuid     = require('uuid/v1')
const pick     = require('lodash.pick')
const endsWith = require('lodash.endswith')
const chalk    = require('chalk')
const connect  = require('./connect')
const DocumentExists   = require('../../errors/DocumentExists')
const DocumentNotFound = require('../../errors/DocumentNotFound')

const { client, rawClient, tablePrefix } = connect()

const Dynamo = AbstractDocument => class extends AbstractDocument {
  static documentId() {
    return uuid()
  }

  // TODO: Remove this method
  static _filterHiddenAttributes(item) {
    return pick(item, Object.keys(item).filter(key => !key.startsWith('_')))
  }

  // TODO: Add support for
  // static get defaultLocalSecondaryIndexSortKey() {
  //   return 'createAt'
  // }

  // TOOD: Add support for
  // static get indexes() {
  //   return []
  // }

  static _buildFilterExpression(filters) {
    let expression   = []
    const attributes = {}
    const values     = {}

    for (let key in filters) {
      let path          = '#' + key.replace(/\./g, '.#')
      const isContains  = endsWith(key, ':contains')
      const valueKey    = key.replace(/\.|:/g, '_')
      const filterValue = filters[key]

      if (isContains) {
        key = key.replace(/:contains/g, '')
      }

      const pathKeys = key.split('.')

      for (const pathKey of pathKeys) {
        attributes[`#${pathKey}`] = pathKey
      }

      const isFilterValueArray = Array.isArray(filterValue)

      if (isFilterValueArray) {
        const filterValues = filterValue.entries()

        for (const [ index, value ] of filterValues) {
          values[`:${valueKey}${index + 1}`] = value
        }

        const valueKeys = Object.keys(values)
        expression.push(`${path} in (${valueKeys})`)

      } else {
        values[`:${valueKey}`] = filterValue

        if (isContains) {
          path = path.replace(/:contains/, '')
          expression.push(`contains(${path}, :${valueKey})`)

        } else {
          expression.push(`${path} = :${valueKey}`)

        }
      }
    }

    expression = expression.join(' and ')

    return {
      attributes,
      expression,
      values
    }
  }

  static async _create(Item) {
    Item.id = this.documentId(Item)

    const parameters = {
      TableName: this.tableName,
      Item: {
        ...Item,
        // TODO: Remove
        _isDeleted: 0
      },
      ConditionExpression:       '#id <> :id',
      ExpressionAttributeNames:  { '#id': 'id' },
      ExpressionAttributeValues: { ':id': Item.id }
    }

    try {
      await client.put(parameters).promise()

    } catch (error) {
      if (error.name == 'ConditionalCheckFailedException') {
        throw new DocumentExists(this.name, { id: Item.id }, error)
      }

      if (error.name == 'ResourceNotFoundException') {
        console.error(chalk`{dim Collection {reset ${this.tableName}} does not exist}`)
      }

      throw error
    }
  }

  static async _index(query, options) {
    const { limit, exclusiveStartKey, sort, indexName } = options || {}

    const IndexName        = indexName ? indexName : 'defaultIndex'
    const ScanIndexForward = sort == 'asc'

    let parameters = {
      TableName: this.tableName,
      IndexName,
      ScanIndexForward,

      // TODO: Remove _isDeleted flag
      KeyConditionExpression:    '#_isDeleted = :_isDeleted',
      ExpressionAttributeNames:  { '#_isDeleted': '_isDeleted' },
      ExpressionAttributeValues: { ':_isDeleted': 0 }
    }

    if (limit) {
      parameters.Limit = limit
    }

    if (exclusiveStartKey) {
      // TODO: Test how this works with different asc / desc when
      //       ExclusiveStartKey is used.

      // TODO: Refactor using querystring:
      //       https://nodejs.org/api/querystring.html

      const buffer = Buffer.from(exclusiveStartKey, 'hex')
      const keyString = buffer.toString('utf-8')
      const [id, createdAt] = keyString.split('|')
      parameters.ExclusiveStartKey = {
        id,
        createdAt,
        // TODO: Remove
        _isDeleted: 0
      }
    }

    const hasQuery = Object.keys(query).length > 0
    if (hasQuery) {
      const {
        expression,
        attributes,
        values
      } = this._buildFilterExpression(query)

      parameters.FilterExpression = expression
      parameters.ExpressionAttributeNames = {
        ...parameters.ExpressionAttributeNames,
        ...attributes
      }
      parameters.ExpressionAttributeValues = {
        ...parameters.ExpressionAttributeValues,
        ...values
      }
    }

    let items = []
    let lastEvaluatedKey

    try {
      // Scan items until get all items or limit
      do {
        const {
          Items,
          LastEvaluatedKey
        } = await client.query(parameters).promise()

        const takenItemsCount = items.length + Items.length

        if (takenItemsCount > limit) {
          // If last chunk has more elements than needed
          const last = items.length + Items.length - limit

          items = [...items, ...Items.slice(0, -last)]
        } else {
          items = [...items, ...Items]
        }

        lastEvaluatedKey = LastEvaluatedKey

        if (items.length < limit && lastEvaluatedKey) {
          parameters.ExclusiveStartKey = lastEvaluatedKey
        }
      } while (items.length < limit && lastEvaluatedKey)

    } catch (error) {
      if (error.name == 'ResourceNotFoundException') {
        console.error(chalk`{dim Collection {reset ${this.tableName}} does not exist}`)
      }

      throw error
    }

    if (lastEvaluatedKey) {
      const { id, createdAt } = lastEvaluatedKey
      const buffer = Buffer.from(`${id}|${createdAt}`, 'utf-8')

      lastEvaluatedKey = buffer.toString('hex')
    }

    return {
      docs:  items.map(item => this._filterHiddenAttributes(item)),
      count: items.length,
      lastEvaluatedKey
    }
  }

  static async _read(id) {
    const parameters = {
      TableName: this.tableName,
      Key:       { id }
    }

    let result
    try {
      result = await client.get(parameters).promise()

    } catch (error) {
      if (error.name == 'ResourceNotFoundException') {
        console.error(chalk`{dim Collection {reset ${this.tableName}} does not exist}`)
      }

      throw error
    }

    const { Item } = result

    // TODO: Remove, probably not used anymore
    if (!Item || Item._isDeleted) {
      throw new DocumentNotFound(this.name)
    }

    return this._filterHiddenAttributes(Item)
  }

  static async _update(id, attributes) {
    const UpdateExpressions         = []
    const ExpressionAttributeNames  = {}
    const ExpressionAttributeValues = { ':id': id }
    const ConditionExpressions      = ['id = :id']

    for (let name in attributes) {
      const isAppend = endsWith(name, ':append')

      const expressionValue = attributes[name]
      name = name.replace(/:append/, '')

      ExpressionAttributeNames[`#${name}`] = name

      if (isAppend) {
        ExpressionAttributeValues[`:${name}`]      = [ expressionValue ]
        ExpressionAttributeValues[`:${name}_item`] = expressionValue

        ConditionExpressions.push(`not contains (#${name}, :${name}_item)`)
        UpdateExpressions.push(`#${name} = list_append(#${name}, :${name})`)

      } else {
        ExpressionAttributeValues[`:${name}`] = attributes[name]

        UpdateExpressions.push(`#${name} = :${name}`)
      }
    }

    // TODO: Remove, probably not used anymore
    if (attributes._isDeleted !== 1) {
      ExpressionAttributeValues[':_isDeleted'] = 0
      ExpressionAttributeNames['#_isDeleted'] = '_isDeleted'
      ConditionExpressions.push('#_isDeleted = :_isDeleted')
    }

    const UpdateExpression = `SET ${UpdateExpressions.join(', ')}`
    const ConditionExpression = ConditionExpressions.join(' and ')

    const parameters = {
      ExpressionAttributeNames,
      ExpressionAttributeValues,
      ConditionExpression,
      UpdateExpression,
      TableName:    this.tableName,
      ReturnValues: 'ALL_NEW',
      Key:          { id }
    }

    let result

    try {
      result = await client.update(parameters).promise()

    } catch (error) {
      if (error.name == 'ConditionalCheckFailedException') {
        throw new DocumentNotFound(this.name)
      }

      if (error.name == 'ResourceNotFoundException') {
        console.error(chalk`{dim Collection {reset ${this.tableName}} does not exist}`)
      }

      throw error
    }

    return result.Attributes
  }

  static async _delete(id) {
    // TODO: Revert this back to how it was before _isDeleted change
    await this._update(id, {
      _isDeleted: 1
    })
  }

  static get tableName() {
    return `${tablePrefix}-${this.name}s`
  }

  static get tableSchema() {
    const TableName = this.tableName

    const AttributeDefinitions = [{
      AttributeName: 'id',
      AttributeType: 'S'
    }, {
      AttributeName: 'createdAt',
      AttributeType: 'S'
    }, {
      AttributeName: '_isDeleted',
      AttributeType: 'N'
    }]

    const KeySchema = [{
      AttributeName: 'id',
      KeyType:       'HASH'
    }]

    // TODO: Support default local secondary index
    // TODO: Support indexes()

    const GlobalSecondaryIndexes = [{
      IndexName: 'defaultIndex',
      KeySchema: [{
        AttributeName: '_isDeleted',
        KeyType: 'HASH',
      }, {
        AttributeName: 'createdAt',
        KeyType: 'RANGE',
      }],
      ProvisionedThroughput: {
        ReadCapacityUnits:  1,
        WriteCapacityUnits: 1
      },
      Projection: {
        ProjectionType: 'ALL',
      },
    }]

    return {
      TableName,
      AttributeDefinitions,
      KeySchema,
      GlobalSecondaryIndexes,
      ProvisionedThroughput: {
        ReadCapacityUnits:  1,
        WriteCapacityUnits: 1
      }
    }
  }

  static async createCollection() {
    try {
      await rawClient.createTable(this.tableSchema).promise()

    } catch (error) {
      if (error.name == 'ResourceInUseException') {
        return
      }

      throw error
    }
  }

  static async deleteCollection() {
    const parameters = {
      TableName: this.tableName
    }

    await rawClient.deleteTable(parameters).promise()
  }

  static async resetCollection() {
    try {
      await this.deleteCollection()

    } catch (error) {
      if (error.code != 'ResourceNotFoundException') {
        throw error
      }
    }

    await this.createCollection()
  }
}

module.exports = Dynamo
