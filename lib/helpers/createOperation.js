...
const createOperation = (operationClassName, Resource, description = '', options = {}) => {
  const OperationClass = DEFAULT_OPERATIONS[operationClassName] || Operation

  const { name: resourceName } = Resource

  const defaultTag     = pluralize(startCase(resourceName))
  const defaultSummary = capitalize(`${operationClassName} ${resourceName}`)
